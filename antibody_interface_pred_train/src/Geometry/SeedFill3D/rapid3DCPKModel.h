 ...
 */
#ifndef RAPID_3D_CPK_MODEL_H_
#define RAPID_3D_CPK_MODEL_H_
...
namespace rapid3DCPKModel {
/**
 * Clears a whole range in temp, from zl to zr, on line y of slice z.
 * @param cpkModel	cpkModel voxelGrid
 * @param zl		range lower bound
 * @param zr		range upper bound
 * @param y			range line
 * @param z			image slice
 */
static inline void clearRange(voxelGrid & cpkModel, int x, int y, int zl, int zr) {
	for (int i = zl; i <= zr; i++)
		cpkModel.clearVoxel(x, y, i);
};
/**
 * Returns the unfilled and valid range discovered scanning the target image from seed
 * @param imageData		input 3D image
 * @param cpkModel		cpkModel 3D image
 * @param seed		the seed
 * OUTPUT:
 * @param zl 		the left range delimiter
 * @param zr 		the right range delimiter
 */
static inline void obtainUnfilledValidRange(voxelGrid const & imageData, voxelGrid const & cpkModel, voxel const & seed,
		int & zl, int & zr) {
	zr = seed.iz;
	while ((zr < imageData.height - 1) && !imageData.getVoxel(seed.ix, seed.iy, zr + 1) && cpkModel.getVoxel(seed.ix, seed.iy, zr + 1)) {
		++zr;
	}
	zl = seed.iz;
	while ((zl > 0) && !imageData.getVoxel(seed.ix, seed.iy, zl - 1) && cpkModel.getVoxel(seed.ix, seed.iy, zl - 1)) {
		--zl;
	}
};
 ...
 */
static inline bool checkAndFillRange(voxelGrid const & imageData, voxelGrid & cpkModel, int zpl, int zpr, int y, int x,
		IdList & dList, int & zl, int & zr) {
	for (int i = zpl; i <= zpr; ++i) {
		if (!imageData.getVoxel(x, y, i) && cpkModel.getVoxel(x, y, i)) { // if a valid seed is found
			...
			obtainUnfilledValidRange(imageData, cpkModel, voxel(x, y, i), zl, zr);
			//extract and fill the range [zl,zr], mark the seeds and write the ID of them into dList
			clearRange(cpkModel, x, y, zl, zr);
 ...
 */
static inline void seedFill2D(voxelGrid const & imageData, voxelGrid & cpkModel, voxel const & seed, IdList & seedList) {
	// empty the seed list
	seedList.clear();
	...
	obtainUnfilledValidRange(imageData, cpkModel, seed, zl, zr);
	// extract and fill the range [zl,zr],
	clearRange(cpkModel, seed.ix, seed.iy, zl, zr);
	...
	while (!stack2D.empty()) { // if the 2D stack is not empty
		...
		while (checkAndFillRange(imageData, cpkModel, zpl, zpr, y, seed.ix, seedList, zl, zr)) { //if the range [zpl, zpr] is valid
			if (zr < zpr - 1) { //it maybe exist other valid and unfilled ranges on the same scan line
				for (int i = zr + 2; i <= zpr; ++i) { // search the leftmost seed of the next valid range, which locates at the same scan line
					if (!imageData.getVoxel(seed.ix, y, i) && cpkModel.getVoxel(seed.ix, y, i)) { /* a valid seed exists */
			...
			// rollback to the previous scan line
				...
				for (int i = zl; i <= zpl - 2; ++i) {
					if (!imageData.getVoxel(seed.ix, old_y, i) && cpkModel.getVoxel(seed.ix, old_y, i)) { /* a valid seed exists */
 ...
 */
static inline bool searchNeighborSlice(voxelGrid const & imageData, voxelGrid const & cpkModel, IdList & seed_list,
	...
	for (id = begin; id != end; ++id) {
		for (int iz = id->zl; iz <= id->zr; iz += leap_var) {
			if (!imageData.getVoxel(x, id->y, iz) && cpkModel.getVoxel(x, id->y, iz)) {
				sz = iz;
				sy = id->y;
				/*
 ...
 * 					valid seed
 */
static inline void cpkModelFill(voxelGrid const & imageData, voxelGrid & cpkModel, voxel const & seed, int leap_var = 1) { // the pseudocode of proposed algorithm — 2009
	...
	seedFill2D(imageData, cpkModel, seed, currentSeeds);
	...
	while (!stack3D.empty()) {
		...
		while(searchNeighborSlice(imageData, cpkModel, currentSeeds, new_x, leap_var, sy, sz)){ //if the region is valid
			IdList newSeeds;
			...
			seedFill2D(imageData, cpkModel, voxel(new_x, sy, sz), newSeeds);
			//continue to search other new unfilled regions in the region (Plist_mapped)
			while (searchNeighborSlice(imageData, cpkModel, currentSeeds, new_x, leap_var, sy, sz)){
				//extract and fill these new unfilled regions
				IdList otherSeeds;
				seedFill2D(imageData, cpkModel, voxel(new_x, sy, sz), otherSeeds);
			...
			while (searchNeighborSlice(imageData, cpkModel, newSeeds, new_x - n.direction_x, leap_var, sy, sz)) {
				IdList otherSeeds;
				//extract and fill these new unfilled regions
				seedFill2D(imageData, cpkModel, voxel(new_x - n.direction_x, sy, sz), otherSeeds);
				//create two nodes with each region, push them into 3D stack, along two opposite directions
