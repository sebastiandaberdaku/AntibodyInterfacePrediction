...
public:
	...
	Molecule(float patchRadius, float probeRadius, float resolution, string const & inname,
	...
	void calculateDescriptors(string const & outname, float minCenterDist_noninterface, float minCenterDist_interface, float patchRadius, int maxOrder, array3D const & interface, vector<CompactPatchDescriptor> & out_descriptors) {
		...
		surface->calculateSurfaceDescriptors(patchRadius, maxOrder, other_patch_centers, interface, nint_d);
		size_t ip = 0, nip = 0;
		...
		surface->calculateSurfaceDescriptors(patchRadius, maxOrder, interface_patch_centers, interface, int_d);
