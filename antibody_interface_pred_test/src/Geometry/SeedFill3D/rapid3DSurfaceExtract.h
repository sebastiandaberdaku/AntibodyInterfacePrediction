 ...
 * @param z			image slice
 */
static inline void clearRange(voxelGrid const & cpkModel, voxelGrid & temp, voxelGrid & output, int const & x, int const & y,
		int const & zl, int const & zr) {
	for (int i = zl; i <= zr; i++) {
		if (!cpkModel.getVoxel(x, y - 1, i) || !cpkModel.getVoxel(x, y + 1, i)
				|| (x > 0 && !cpkModel.getVoxel(x - 1, y, i))
				|| (x < output.length - 1 && !cpkModel.getVoxel(x + 1, y, i)))
			output.setVoxel(x, y, i);
		temp.clearVoxel(x, y, i);
	}
		output.setVoxel(x, y, zl);
		output.setVoxel(x, y, zr);
};
 ...
 */
static inline bool checkAndClearRange(voxelGrid const & cpkModel, voxelGrid & temp, voxelGrid & output, int zpl, int zpr, int y, int x,
	...
	for (int i = zpl; i <= zpr; ++i) {
		if (temp.getVoxel(x, y, i)) { // if a valid seed is found
			...
			obtainFilledValidRange(temp, voxel(x, y, i), zl, zr);
			//extract and fill the range [zl,zr], mark the seeds and write the ID of them into dList
			clearRange(cpkModel, temp, output, x, y, zl, zr);
...
static inline void surfaceExtract2D(voxelGrid const & cpkModel, voxelGrid & temp, voxelGrid & output, voxel const & seed, IdList & seedList) {
	...
	obtainFilledValidRange(temp, seed, zl, zr);
	// extract and fill the range [zl,zr],
	clearRange(cpkModel, temp, output,  seed.ix, seed.iy, zl, zr);
	...
	while (!stack2D.empty()) { // if the 2D stack is not empty
		...
		while (checkAndClearRange(cpkModel, temp, output, zpl, zpr, y, seed.ix, seedList, zl, zr)) { //if the range [zpl, zpr] is valid
 ...
 */
static inline bool searchNeighborSlice(voxelGrid const & temp, IdList & seed_list, int x, int leap_var, int & sy, int & sz) {
...
static inline void surfaceExtract3D(voxelGrid const & imageData, voxelGrid & temp, voxelGrid & output, voxel const & seed, int leap_var = 1) { // the pseudocode of proposed algorithm — 2009
	...
	while (!stack3D.empty()) {
		...
		while(searchNeighborSlice(temp, currentSeeds, new_x, leap_var, sy, sz)){ //if the region is valid
			...
			surfaceExtract2D(imageData, temp, output, voxel(new_x, sy, sz), newSeeds);
			...
			while (searchNeighborSlice(temp, currentSeeds, new_x, leap_var, sy, sz)){
				...
				IdList otherSeeds;
				surfaceExtract2D(imageData, temp, output, voxel(new_x, sy, sz), otherSeeds);
			...
			while (searchNeighborSlice(temp, newSeeds, new_x - n.direction_x, leap_var, sy, sz)) {
				IdList otherSeeds;
				//extract and fill these new unfilled regions
				surfaceExtract2D(imageData, temp, output, voxel(new_x - n.direction_x, sy, sz), otherSeeds);
				//create two nodes with each region, push them into 3D stack, along two opposite directions
